...
cars_data %>%
  ...
  hc_title(text = "Collsions By Injuriy Severity", align = "center") |>
...
cars_data %>%
  ...
  hc_title(text = "Collsions By Offense Type", align = "center") |>
  hc_tooltip(pointFormat = "{series.name}: <b>{point.y:.1f}%</b>") |>
  hc_xAxis(title = NULL, labels = list(enabled = TRUE)) %>%
  hc_plotOptions(column = list(dataLabels = list(enabled = TRUE, format = '{point.y:.1f}%'),
                               color = "#3498db")) %>%
...
cars_data %>%
  ...
  summarise(
    ...
    crosswalk = sum(violation_crosswalk == "Yes", na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(
  ...
  hc_plotOptions(
    series = list(
      ...
      dataLabels = list(
        enabled = TRUE, 
        format = '{y}', 
        style = list(fontSize = "8px")  # Reduced font size for data labels
      )
    )
  ) %>%
  hc_xAxis(
    title = NULL,
    categories = unique(.$year),
    labels = list(
      rotation = 0,
...
cars_data %>%
  ...
  hc_xAxis(
    title = NULL,
    categories = unique(.$year), # Use unique years as categories
    labels = list(
      rotation = 0, # Rotate x-axis labels
...
```{r}
#| title: Collision Manner

cars_data |>
  group_by(year, collision_manner) |>
  ...
  select(collision_manner, everything()) |>
  kable("html") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F) |>
  column_spec(1, bold = T)
...
cars_data |>
  ...
  hchart("line", hcaes(x = year, y = count, group = metric)) |>
  hc_title(text = "Car Collsions Trends (Yearly)", align = "center") |>
  hc_xAxis(title = NULL) |>
...
cars_data |>
  ...
  hc_title(text = "Car Collsions Trends (Monthly)", align = "center") |>
  hc_xAxis(title = NULL) |>
  hc_yAxis(title = list(text = "Count")) |>
  hc_legend(
    enabled = TRUE,
...
cars_data |>
  ...
  hc_title(text = "Car Collsions Trends (Weekly)", align = "center") |>
  hc_xAxis(title = NULL) |>
  hc_yAxis(title = list(text = "Total Count")) |>
  hc_legend(
    enabled = TRUE,
...
cars_data |>
  ...
  hc_title(text = "Collsions Trends (Hourly)", align = "center") |>
  hc_xAxis(title = NULL) |> 
  hc_yAxis(title = list(text = "Total Count")) |>
  hc_legend(
    enabled = TRUE,
...
cars_data |>
  ...
  hc_colors(c("#3498db", "#e74c3c", "#2ecc71")) |>
  hc_title(text = "Car Collsions By Division", align = "center") |>
  hc_tooltip(
...
cars_data |>
  ...
  hc_colors(c("#3498db", "#e74c3c", "#2ecc71")) |>
  hc_title(text = "Car Collsions By Ward", align = "center") |>
  hc_tooltip(
    pointFormat = "{point.name}:: <b>{point.y}</b>",
    formatter = JS("function() { return Highcharts.numberFormat(this.y, 0, '.', ','); }")) |>
  hc_plotOptions(column = list(pointPadding = 0.04,
...
cars_data |>
  ...
  hc_colors(c("#3498db", "#e74c3c", "#2ecc71")) |>
  hc_title(text = "Car Collsions By Neighbourhood", align = "center") |>
  hc_tooltip(
    pointFormat = "{point.name}:: <b>{point.y}</b>",
    formatter = JS("function() { return Highcharts.numberFormat(this.y, 0, '.', ','); }")) |>
  hc_plotOptions(column = list(pointPadding = 0.04,
...
cars_data |>
  ...
  hc_colors(c("#3498db", "#e74c3c", "#2ecc71")) |>
  hc_title(text = "Car Collsions By Location", align = "center") |>
  hc_tooltip(
    pointFormat = "{point.name}:: <b>{point.y}</b>",
    formatter = JS("function() { return Highcharts.numberFormat(this.y, 0, '.', ','); }")) |>
  hc_plotOptions(column = list(pointPadding = 0.04,
