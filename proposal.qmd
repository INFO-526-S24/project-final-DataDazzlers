...
tibble(
  "Column Name" = names(crash_data),  # considering only the first five columns
  "Data Type" = sapply(crash_data, class),  # considering only the first five columns
  "Unique 5 Value" = sapply(crash_data, function(column) {
    first_five_unique_values <- unique(column[!is.na(column)])[1:5]  # extract the first five unique non-NA values
    if (all(sapply(first_five_unique_values, is.numeric))) {  # check if all first five unique values are numeric
...
Plots to use: To answer this question, we plan to create daily and hourly time series plots to easily observe trends in accident frequency and injury severity. In the Shiny app, there can be a time series bar where the user drags the button and the plot shows the different numbers of accidents in different hours in a day and days in the week.
...
-   Week of **03 April**
    ...
    -   Provide peer review for other groups in class.
-   Week of **08 April**
    -   Make changes to proposal based on peer review for instructor review and resubmit.
-   Week of **15 April**
...
-   Week of **22 April, 29 April**
    ...
    -   Discuss how to implement these visualizations into the Shiny app.
    -   Complete write up.
